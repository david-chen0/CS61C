...
/* NUMBER METHODS */
static int alloc_check(int error_code){
    if (error_code == -1){
        PyErr_SetString(PyExc_ValueError, "Dimensions must be positive and nonzero!");
        return -1;
...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    // Throw type error if self and b are not of type numc.Matrix
    ...
    // Casting PyObject to Matrix61C
    Matrix61c* rv = (Matrix61c*) args;
    // Throw ValueError if matrices don't have same dimensions
    if (self->mat->rows != rv->mat->rows || self->mat->cols != rv->mat->cols) {
        PyErr_SetString(PyExc_ValueError, "Dimensions must be equal!");
        return NULL;
    }
    // Create resultant matrix
    matrix *res_mat;
    int alloc_code = allocate_matrix(&res_mat, self->mat->rows, self->mat->cols);

    // Checking if allocation worked: 0 for success; -1,-2 for failure
    if (alloc_check(alloc_code) == 0){
        add_matrix(res_mat, self->mat, rv->mat);
        Matrix61c* res = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
        res->mat = res_mat;
        res->shape = PyTuple_Pack(2, PyLong_FromLong(res_mat->rows), PyLong_FromLong(res_mat->cols));
        return (PyObject*)res;
    }
    return NULL;
}
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    // Throw ValueError if matrices don't have same dimensions
    if (self->mat->rows != rv->mat->rows || self->mat->cols != rv->mat->cols) {
        PyErr_SetString(PyExc_ValueError, "Dmensions must be equal!");
        return NULL;
    }
    // Create resultant matrix
    matrix *res_mat;
    ...
    // Checking if allocation worked: 0 for success; -1,-2 for failure
    if (alloc_check(alloc_code) == 0){
        sub_matrix(res_mat, self->mat, rv->mat);
        Matrix61c* res = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
        res->mat = res_mat;
        res->shape = PyTuple_Pack(2, PyLong_FromLong(res_mat->rows), PyLong_FromLong(res_mat->cols));
        return (PyObject*)res;
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    Matrix61c* rv = (Matrix61c*) args;
    // Throw ValueError if matrices don't have same dimensions
    if (self->mat->cols != rv->mat->rows) {
        PyErr_SetString(PyExc_ValueError, "Dimensions do not match!");
        return NULL;
    }
    // Create resultant matrix
    matrix *res_mat;
    int alloc_code = allocate_matrix(&res_mat, self->mat->rows, rv->mat->cols);
    // Checking if allocation worked: 0 for success; -1,-2 for failure
    if (alloc_check(alloc_code) == 0){
        mul_matrix(res_mat, self->mat, rv->mat);
        Matrix61c* res = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
        res->mat = res_mat;
        res->shape = PyTuple_Pack(2, PyLong_FromLong(res_mat->rows), PyLong_FromLong(res_mat->cols));
        return (PyObject*)res;
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    // Throw type error if self is not of type numc.Matrix
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    // Throw error if pow is not an int
    if (!PyLong_Check(pow)) {
        PyErr_SetString(PyExc_TypeError, "Power must be an integer!");
        return NULL;
    }
    int pw = PyLong_AsLong(pow);
    // Throw error if pow is negative
    if (pw < 0) {
        PyErr_SetString(PyExc_ValueError, "Power must be non-negative!");
        return NULL;
    }
    // Throw error is self is not a square matrix
    ...
    // Checking if allocation worked: 0 for success; -1,-2 for failure
    if (alloc_check(alloc_code) == 0){
        pow_matrix(res_mat, self->mat, pw);
        Matrix61c* res = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
        res->mat = res_mat;
        res->shape = PyTuple_Pack(2, PyLong_FromLong(res_mat->rows), PyLong_FromLong(res_mat->cols));
        return (PyObject*)res;
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 3, &arg1, &arg2, &arg3)){
        ...
        if (arg1 && arg2 && arg3 && PyLong_Check(arg1) && PyLong_Check(arg2) && (PyLong_Check(arg3) || PyFloat_Check(arg3))) {
            long a1 = PyLong_AsLong(arg1);
            long a2 = PyLong_AsLong(arg2);
            double a3 = PyFloat_AsDouble(arg3);
            // Out of range
            if (a1 < 0 || a1 >= self->mat->rows || a2 < 0 || a2 >= self->mat->cols) {
                PyErr_SetString(PyExc_IndexError, "Index out of range");
                return NULL;
            }
            set(self->mat, a1, a2, a3);
            Py_IncRef(Py_None);
            return Py_None;              
        } else {
            PyErr_SetString(PyExc_TypeError, "Invalid arguments");
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 2, &arg1, &arg2)){
        ...
        if (arg1 && arg2 && PyLong_Check(arg1) && PyLong_Check(arg2)) {
            ...
            return PyFloat_FromDouble(get(self->mat, a1, a2));              
        } else {
            PyErr_SetString(PyExc_TypeError, "Invalid arguments");
            return NULL;
        }
...
static PyMethodDef Matrix61c_methods[] = {
    {"get", (PyCFunction)Matrix61c_get_value, METH_VARARGS, "Returns value in matrix"},
    {"set", (PyCFunction)Matrix61c_set_value, METH_VARARGS, "Sets a value in matrix"},
    {NULL, NULL, 0, NULL}
};
