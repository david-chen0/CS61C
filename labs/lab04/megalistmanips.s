...
mapLoop:
    lw t1, 0(s0)      # load the address of the array of current node into t1 THIS LINE WAS A BUG
    lw t2, 4(s0)        # load the size of the node's array into t2

	slli t3, t0, 2      # offset needs to be multiplied by 4 since 4 bytes for each input THIS LINE WAS ADDED IN
    add t1, t1, t3      # offset the array address by the count THIS LINE WAS A BUG
    lw a0, 0(t1)        # load the value at that address into a0

    addi sp, sp, -12 # THIS LINE WAS ADDED IN
    sw t0, 0(sp) # THIS LINE WAS ADDED IN
    sw t1, 4(sp) # THIS LINE WAS ADDED IN
    sw t2, 8(sp) # THIS LINE WAS ADDED IN
    
    jalr s1             # call the function on that value.
    
    lw t0, 0(sp) # THIS LINE WAS ADDED IN
    lw t1, 4(sp) # THIS LINE WAS ADDED IN
    lw t2, 8(sp) # THIS LINE WAS ADDED IN
    addi sp, sp, 12 # THIS LINE WAS ADDED IN
    ...
    lw a0, 8(s0)        # load the address of the next node into a0 THIS LINE WAS A BUG
    add a1, s1, x0        # put the address of the function back into a1 to prepare for the recursion THIS WAS A BUG
